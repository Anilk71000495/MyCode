...
pdfjsLib.GlobalWorkerOptions.workerSrc =
  ...
  loadingTask.promise.then(function(pdf) {
    ...
    //
	 ...
	 var numPages = pdf.numPages;
            $("#hdnMaxPage").val(numPages);
            $("#spnTotalPage").text("of " + numPages);
            if (parseInt(numPages) > 1) {
                $("#pager").removeClass("sr-only");
                $("li.page-item[state='prev']").addClass("disabled");
            }
      ...
      onmove: dragMoveListener,
		...
		if (interaction.pointerIsDown && !interaction.interacting() && event.currentTarget.getAttribute('clonable') != 'false') {
		  ...
		  var y = clone.offsetTop;
		   var _newUniqueClass = _getUniqueClass();
		$(clone).attr('guid', _newUniqueClass);
  ...
  $("li.page-item").click(function () {
            var _maxP = parseInt($("#hdnMaxPage").val());
            var _cP = parseInt($("#hdnCurrentPage").val());
            var _nP = _cP + 1;
            var _pP = _cP - 1;
            var prevnext = $(this).attr("state");

            if (prevnext != undefined) { /// prev and next button
                switch (prevnext) {
                    case "prev":
                        if (_cP > 1) {
                            if (_pP > 1) {
                                $("li.page-item[state='prev']").removeClass("disabled");
                                $("li.page-item[state='next']").removeClass("disabled");
                                $("#hdnCurrentPage").val(_pP);
                                $("#spnPage").text(_pP);
                                ChangePage(_pP);
                            } else {
                                $("li.page-item[state='prev']").addClass("disabled");
                    ...
                    case "next":
                        if (_cP == 1) {
                            if (_nP < _maxP) {
                                $("li.page-item[state='prev']").removeClass("disabled");
                                $("#hdnCurrentPage").val(_nP);
                                $("#spnPage").text(_nP);
                                ChangePage(_nP);
                            } else {
                                $("li.page-item[state='next']").addClass("disabled");
                                $("li.page-item[state='prev']").removeClass("disabled");
                                $("#hdnCurrentPage").val(_nP);
                                $("#spnPage").text(_nP);
                                ChangePage(_nP);
        ...
        function ChangePage(pg) {
           // EnableLoader();
            loadingTask.promise.then(function (pdf) {
                var numPages = pdf.numPages;
                pdf.getPage(parseInt(pg)).then(function (page) {
                    var scale = 1.0;
                    var viewport = page.getViewport(scale);
                    var canvas = document.getElementById('the-canvas');
                    var context = canvas.getContext('2d');
		...
		function DisplayCoordinates(){
			//debugger;
		var acc = []
		$.each(coordinatesArr, function(index, value) {
			acc.push('[coordX : ' + value.coordX +' ,coordY : ' + value.coordY + ' ,Page : ' + value.page + ' ,Id : ' + value.id+ ' ,Value : ' + value.valore +'],');
		});
		alert(JSON.stringify(acc));
		}
   function showCoordinatesNew() { 
            ...
            $('.drag-drop.can-drop').each(function (index) {
				var _valore = $(this).attr('data-valore');
                ...
                for (var i = 0; i < coordinatesArr.length; i++) {
                    if (coordinatesArr[i].id.indexOf($(this).attr("guid")) > -1) {
                        isexists = true;
                        coordinatesArr[i].coordX = posizioneX;
                        coordinatesArr[i].coordY = posizioneY;
						coordinatesArr[i].htmlX = 	x;				
						coordinatesArr[i].htmlY = 	y;
                    }
                }
                if (!isexists) {
                    var c = new Object();
                    c.page = $("#hdnCurrentPage").val();
                    c.id = $(this).attr("guid");
                    c.coordX = posizioneX;
                    c.coordY = posizioneY; 
					c.valore = _valore;					
